"""
VDN GroupDR algorithm: VDN training that uses a pre-trained EnvEstimator to provide
...
class VDNGroupDR:
	"""
	VDN with distributional robustness using EnvEstimator for per-step reward.
	- Behavior: standard VDN mixing and target computation
	...
	def __init__(self, env_estimator_path: str, device=None):
		...
		# configs
		self.training_params = get_training_params('vdn_groupdr')
	...
	def _initialize_networks(self):
		q_nets = [
			QNetwork(obs_dim, self.agent_info['n_actions'], hidden_dim=self.network_params['hidden_dim']).to(self.device)
	...
	def _initialize_optimizers(self):
		return [
			torch.optim.RMSprop(
				q_net.parameters(), lr=self.network_params['learning_rate']['q_networks'], eps=1e-5
			)
	...
	def _log_training_progress(self, loss, global_q, target):
		log_file = create_log_file('vdn_groupdr')
	...
	def train(self):
		...
		seeds = seeds.tolist()
		pbar = tqdm(range(self.training_params['num_episodes']), desc='Training VDN-GroupDR')
		for episode in pbar:
			...
			# log episode reward
			with open('vdn_groupdr_reward.txt', 'a') as f:
	...
	def _save_results(self):
		...
		results_dir = create_results_dir('vdn_groupdr', 0.0)
		...
		plt.xlabel('Episode')
		plt.ylabel('Reward')
		plt.title('VDN-GroupDR Training Reward')
		plt.legend()
