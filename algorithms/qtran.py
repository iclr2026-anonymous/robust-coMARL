...
from networks.networks import QNetworkWithPhi, JointActionValueNetwork, StateValueNetwork
from utils.replay_buffer import ReplayBuffer
from utils.evaluation import select_actions, parallel_evaluate_policy

class RobustQTRAN:
    """
    Robust QTRAN algorithm implementation with contamination robustness.
    """
    
    def __init__(self, rho_value, device=None):
        """
        Initialize Robust QTRAN algorithm.
        ...
        self.training_params = get_training_params('qtran')
    ...
    def _initialize_optimizer(self):
        ...
        return torch.optim.Adam(all_params, lr=self.network_params['learning_rate']['g_network'])
    ...
    def _update_networks(self):
        ...
        with torch.no_grad():
            ...
            target = reward_batch + (1-self.rho_value)*self.network_params['gamma'] * next_q_jt * (1 - done_batch)
    ...
    def _log_training_progress(self, loss, L_td, L_opt, L_nopt):
        ...
        log_file = create_log_file('qtran')
    ...
    def train(self):
        ...
        pbar = tqdm(range(self.training_params['num_episodes']), 
                   desc=f'Training QTRAN (rho={self.rho_value})')
        ...
        for episode in pbar:
            ...
            # Save episode reward
            with open('qtran_reward.txt', 'a') as f:
    ...
    def _save_results(self):
        ...
        results_dir = create_results_dir('qtran', self.rho_value)
        ...
        plt.xlabel('Episode')
        plt.ylabel('Reward')
        plt.title(f'Robust QTRAN Training Reward (rho={self.rho_value})')
        plt.legend()
