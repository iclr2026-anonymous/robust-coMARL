"""
QMIX with G-Network algorithm implementation with contamination robustness.
"""
...
from networks.networks import QNetwork, QMixer, GNetwork
from utils.replay_buffer import ReplayBuffer
from utils.evaluation import select_actions, parallel_evaluate_policy

class RobustQMIXG:
    """
    Robust QMIX with DUAL network implementation with contamination robustness.
    """
    
    def __init__(self, rho_value, device=None):
        """
        Initialize Robust QMIX with DUAL network algorithm.
        ...
        self.training_params = get_training_params('qmix_g')
        self.network_params = get_network_params()
        self.algorithm_params = get_algorithm_params('qmix_g')
    ...
    def _initialize_optimizers(self):
        ...
        all_optimizer = torch.optim.Adam(all_params, lr=0.0005)
    ...
    def _update_g_network(self):
        ...
        next_action_histories_batch = [
            torch.stack([torch.FloatTensor(transition[5][i]) for transition in batch]).to(self.device)
            for i in range(self.agent_info['n_agents'])
        ]
    ...
    def _log_g_training_progress(self, g_loss, g_values):
        ...
        log_file = create_log_file('qmix_g')
    ...
    def train(self):
        ...
        pbar = tqdm(range(self.training_params['num_episodes']), 
                   desc=f'Training QMIX+G (rho={self.rho_value})')
    ...
    def _save_results(self):
        ...
        results_dir = create_results_dir('qmix_g', self.rho_value)
        ...
        plt.xlabel('Episode')
        plt.ylabel('Reward')
        plt.title(f'Robust QMIX+G Training Reward (rho={self.rho_value})')
        plt.legend()
