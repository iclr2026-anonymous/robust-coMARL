"""
QTRAN with G-Network algorithm implementation with contamination robustness.
"""
...
from networks.networks import QNetworkWithPhi, JointActionValueNetwork, StateValueNetwork, GNetworkWithHidden
from utils.replay_buffer import ReplayBuffer
from utils.evaluation import select_actions, parallel_evaluate_policy

class RobustQTRANG:
    """
    Robust QTRAN with G-Network algorithm implementation with contamination robustness.
    """
    
    def __init__(self, rho_value, device=None):
        """
        Initialize Robust QTRAN with G-Network algorithm.
        ...
        self.training_params = get_training_params('qtran_g')
        self.network_params = get_network_params()
        self.algorithm_params = get_algorithm_params('qtran_g')
        ...
        self.all_optimizer, self.g_optimizer = self._initialize_optimizers()
    ...
    def _initialize_optimizers(self):
        ...
        all_optimizer = torch.optim.Adam(all_params, lr=0.001)
        g_optimizer = torch.optim.Adam(self.g_net.parameters(), lr=0.001)
        
        return all_optimizer, g_optimizer
    ...
    def _update_g_network(self):
        ...
        next_v_jt = next_q_jt.max(dim=1)[0]
        
        # Get G-values
        g_values = self.g_net(state_batch, joint_actions_batch)
        
        # Compute G-loss (robust objective)
        tmp1 = torch.relu(g_values.squeeze(-1) - next_v_jt)
        ...
        g_loss.backward()
        torch.nn.utils.clip_grad_norm_(self.g_net.parameters(), max_norm=1.0)
        self.g_optimizer.step()
    
    def _update_q_networks(self):
        """Update Q-networks and V-network (QTRAN style)."""
        if not self.buffer.is_ready(self.network_params['batch_size']):
            return
        ...
        # Compute Q-values and hidden features
        all_q_outputs = [
        ...
        tmp1 = torch.relu(g_values.squeeze(-1) - next_q_jt)
        ...
        if self.global_step % 1000 == 0:
            self._log_q_training_progress(loss, L_td, L_opt, L_nopt)
        
        # Update networks
        self.all_optimizer.zero_grad()
        loss.backward()
        torch.nn.utils.clip_grad_norm_(self.all_optimizer.param_groups[0]['params'], max_norm=1.0)
        self.all_optimizer.step()
    
    def _log_g_training_progress(self, g_loss, g_values):
        """Log G-network training progress to file."""
        log_file = create_log_file('qtran_g')
    ...
    def _log_q_training_progress(self, loss, L_td, L_opt, L_nopt):
        """Log Q-network training progress to file."""
        log_file = create_log_file('qtran_g')
        with open(log_file, 'a') as f:
            f.write(f"[Step {self.global_step}] q_loss: {loss.item():.6f}\n")
    ...
    def train(self):
        ...
        pbar = tqdm(range(self.training_params['num_episodes']), 
                   desc=f'Training QTRAN+G (rho={self.rho_value})')
        ...
        for episode in pbar:
            ...
            while not done:
                ...
                if self.global_step % self.training_params['update_freq'] == 0:
                    # Update G-network
                    for _ in range(self.algorithm_params.get('number_update_g_steps', 1)):
                        self._update_g_network()
                    
                    # Update Q-networks and V-network
                    for _ in range(self.algorithm_params.get('number_update_q_steps', 5)):
            ...
            with open('qtran_g_rewards.txt', 'a') as f:
                f.write(f"[Episode {episode}] train_rewards: {episode_reward:.6f}\n")
    ...
    def _save_results(self):
        ...
        results_dir = create_results_dir('qtran_g', self.rho_value)
        ...
        plt.xlabel('Episode')
        plt.ylabel('Reward')
        plt.title(f'Robust QTRAN+G Training Reward (rho={self.rho_value})')
        plt.legend()
