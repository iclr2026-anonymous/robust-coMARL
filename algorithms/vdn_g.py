"""
VDN with G-Network algorithm implementation with contamination robustness.
"""
...
from configs.config import TRAINING_CONFIG, EVAL_CONFIGS, get_training_params, get_network_params, create_results_dir, create_log_file, get_algorithm_params
from environments.env_utils import create_environment, process_obs_dict_partial, pad_history, pad_action_history
from networks.networks import QNetwork, GNetwork
from utils.replay_buffer import ReplayBuffer
from utils.evaluation import select_actions, parallel_evaluate_policy
from configs.config import sample_env_config_from_all

class RobustVDNG:
    """
    Robust VDN with G-Network algorithm implementation with contamination robustness.
    """
    
    def __init__(self, rho_value, device=None):
        """
        Initialize Robust VDN with G-Network algorithm.
        ...
        self.training_params = get_training_params('vdn_g')
        self.network_params = get_network_params()
        self.algorithm_params = get_algorithm_params('vdn_g')
        ...
        # Create environment
        if self.algorithm_params.get('randomize_env_from_configs', False):
            self.env = create_environment(sample_env_config_from_all())
        else:
        ...
        self.g_net = self._initialize_g_network()
    ...
    def _initialize_g_network(self):
    ...
    def _initialize_optimizers(self):
        """Initialize optimizers for Q-networks and G-network."""
        q_optimizers = [
            torch.optim.RMSprop(
                q_net.parameters(), 
                lr=self.network_params['learning_rate']['q_networks'],
                eps=1e-5
        ...
        g_optimizer = torch.optim.Adam(
            ...
            lr=self.network_params['learning_rate']['g_network']
        )
        
        return q_optimizers, g_optimizer
    
    def _initialize_mixer(self):
        """Initialize VDN mixer - simple sum of Q-values."""
        # VDN uses simple sum of individual Q-values
        return None  # We'll implement the mixing logic directly
    
    def _mix_q_values(self, q_values):
        """Mix Q-values using VDN (simple sum)."""
        return torch.stack(q_values, dim=1).sum(dim=1)
    ...
    def _update_g_network(self):
        """Update G-network using experience replay."""
        ...
        joint_actions_batch = actions_batch
        ...
        global_next_max_q = self._mix_q_values(next_max_q_values)
        
        # Get G-values
        g_values = self.g_net(state_batch, joint_actions_batch)
        
        # Compute G-loss
        tmp1 = torch.relu(g_values.squeeze(-1) - global_next_max_q)
        tmp2 = tmp1 - (1 - self.rho_value) * g_values.squeeze(-1)
        g_loss = torch.mean(torch.square(tmp2))
        
        # Save G-loss periodically
        if self.global_step % 1000 == 0:
            self._log_g_training_progress(g_loss, g_values)
        
        # Minimize G-loss
        self.optimizers[1].zero_grad()  # g_optimizer is the second optimizer
        g_loss.backward()
        self.optimizers[1].step()
    
    def _update_q_networks(self):
        """Update Q-networks using experience replay."""
        if not self.buffer.is_ready(self.network_params['batch_size']):
            return
        ...
        global_q = self._mix_q_values(chosen_q_values)
        
        # Get G-values
        g_values = self.g_net(state_batch, joint_actions_batch)
        
        # Compute target using G-network
        with torch.no_grad():
            next_max_q_values = [
                target_net(next_obs_batch[i], next_action_histories_batch[i].unsqueeze(-1))[0].max(dim=1)[0]
                for i, target_net in enumerate(self.target_nets)
        ...
        tmp1 = torch.relu(g_values.squeeze(-1) - global_next_max_q)
        target = reward_batch + self.network_params['gamma'] * ((1 - self.rho_value) * g_values.squeeze(-1) - tmp1) * (1 - done_batch)
        
        # Compute Q-loss
        q_loss = nn.MSELoss()(global_q, target)
        
        # Save Q-loss periodically
        if self.global_step % 1000 == 0:
            self._log_q_training_progress(q_loss, global_q, target)
        
        # Update Q-networks
        for opt in self.optimizers[0]:  # q_optimizers is the first optimizer
            opt.zero_grad()
        q_loss.backward()
        for opt in self.optimizers[0]:  # q_optimizers is the first optimizer
            opt.step()
    
    def _log_g_training_progress(self, g_loss, g_values):
        """Log G-network training progress to file."""
        log_file = create_log_file('vdn_g')
        with open(log_file, 'a') as f:
            f.write(f"[Step {self.global_step}] g_loss: {g_loss.item():.6f}\n")
            f.write(f"[Step {self.global_step}] G-values stats: mean={g_values.mean().item():.4f}, "
                   f"min={g_values.min().item():.4f}, max={g_values.max().item():.4f}\n")
    
    def _log_q_training_progress(self, q_loss, global_q, target):
        """Log Q-network training progress to file."""
        log_file = create_log_file('vdn_g')
        with open(log_file, 'a') as f:
            f.write(f"[Step {self.global_step}] q_loss: {q_loss.item():.6f}\n")
    ...
    def train(self):
        ...
        pbar = tqdm(range(self.training_params['num_episodes']), 
                   desc=f'Training VDN+G (rho={self.rho_value})')
        
        for episode in pbar:
            # Optional: resample environment per episode from all configs
            if self.algorithm_params.get('randomize_env_from_configs', False) and self.algorithm_params.get('randomize_per_episode', True):
                self.env = create_environment(sample_env_config_from_all())
                # Agent info depends on env, refresh it to be safe (dimensions are constant across provided configs)
                self.agent_info = self._get_agent_info()
            ...
            while not done:
                ...
                if self.global_step % self.training_params['update_freq'] == 0:
                    # Update G-network
                    for _ in range(self.algorithm_params.get('number_update_g_steps', 2)):
                        self._update_g_network()
                    
                    # Update Q-networks
                    for _ in range(self.algorithm_params.get('number_update_q_steps', 4)):
    ...
    def _save_results(self):
        ...
        results_dir = create_results_dir('vdn_g', self.rho_value)
        ...
        plt.xlabel('Episode')
        plt.ylabel('Reward')
        plt.title(f'Robust VDN+G Training Reward (rho={self.rho_value})')
        plt.legend()
