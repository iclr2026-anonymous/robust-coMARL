"""
VDN (Value Decomposition Networks) algorithm implementation with rho-contamination robustness.
"""
...
class VDN_DOMAIN_RANDOMIZATION:
    """
    Robust VDN algorithm implementation with contamination robustness.
    """
    
    def __init__(self, device=None):
        """
        Initialize Robust VDN algorithm.
        ...
        self.training_params = get_training_params('vdn_dr')
    ...
    def _initialize_optimizers(self):
        """Initialize optimizers for Q-networks."""
        return [
            torch.optim.RMSprop(
                q_net.parameters(), 
    ...
    def _initialize_mixer(self):
        ...
        from torchrl.modules import VDNMixer
        return VDNMixer(n_agents=self.agent_info['n_agents'], device=self.device)
    
    def _is_valid_transition(self, transition):
        """Check if a transition tuple is valid."""
        if len(transition) != 7:
            return False
        obs_histories, action_histories, actions, _, next_obs_histories, next_action_histories, _ = transition
    ...
    def _select_actions(self, obs_histories, action_histories):
        ...
        if self.action_rng.random() < epsilon:
            return [self.action_rng.integers(self.agent_info['n_actions']) for agent in self.agent_info['agent_list']]
        else:
    ...
    def _update_networks(self):
        ...
        global_q = self.mixer(chosen_q_stack).squeeze(-1)
        ...
        with torch.no_grad():
            ...
            global_next_max_q = self.mixer(next_max_q_stack).squeeze(-1)
            target = reward_batch + self.network_params['gamma'] * global_next_max_q * (1 - done_batch)
    ...
    def train(self):
        ...
        pbar = tqdm(range(self.training_params['num_episodes']), 
                   desc=f'Training VDN Domain Randomization')  
        
        for episode in pbar:
            # randomly select environment configuration
            env_idx = self.env_rng.integers(0, len(DR_CONFIGS))
            env_config = DR_CONFIGS[env_idx]
            
            # create new environment and set seed
            self.env = create_environment(env_config)
            
            # set different environment seed for each episode (based on global seed and episode number)
            ...
            while not done:
                ...
                self._store_transition(
                    ...
                    next_obs_histories, next_action_histories, done, None, None
                )
    ...
    def _evaluate_policy(self, episode):
        ...
        for eval_config in EVAL_CONFIGS:
            ...
            print(f"Episode {episode} Eval on {eval_config['name']}: "
    ...
    def _save_results(self):
        ...
        results_dir = create_results_dir('vdn_dr',0.0)
        ...
        plt.xlabel('Episode')
        plt.ylabel('Reward')
        plt.title(f'VDN Domain Randomization Training Reward')
        plt.legend()
